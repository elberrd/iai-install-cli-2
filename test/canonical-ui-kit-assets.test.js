...
test('canonical DataTable and Kanban preserve the professional interaction floor', { skip }, () => {
  ...
  assert.match(table, /advancedControls\s*=\s*true/);
  assert.match(table, /enableColumnReordering\s*=\s*true/);
  assert.match(table, /moveColumnToDropTarget/);
  assert.match(table, /columnDragRef/);
  assert.match(table, /globalFilter/);
  assert.match(table, /aria-label=\{resolvedSearchPlaceholder\}/);
  ...
  assert.match(table, /manualGrouping:\s*true/);
  assert.match(table, /groupingAdapter/);
  assert.match(table, /getSubRows/);
  assert.match(table, /isGroupRow/);
  assert.match(table, /getLeafCount/);
  assert.match(table, /groupingAvailable/);
  assert.match(table, /grouping,\s*expanded/);
  assert.match(table, /serverQueryRef\.current\?\.\(\{ globalFilter, columnFilters, sorting, grouping, expanded, pagination \}\)/);
  ...
  assert.match(table, /tableId/);
  assert.match(table, /persistence\s*=\s*true/);
  assert.match(table, /userId/);
  assert.match(table, /tenantId/);
  assert.match(table, /schemaVersion/);
  assert.match(table, /window\.location\.pathname/);
  assert.match(table, /sanitizePersistedView/);
  assert.match(table, /memoryViewStorage/);
  assert.match(table, /readPersistedView/);
  assert.match(table, /writePersistedView/);
  assert.match(table, /removePersistedView/);
  assert.match(table, /useClientLayoutEffect/);
  assert.match(table, /hydratedStorageKey === persistenceTarget/);
  assert.match(table, /if \(!effectiveStorageKey \|\| !hydratedView\) return/);
  assert.match(table, /skipPersistenceWriteKeyRef/);
  assert.match(table, /inert=\{!hydratedView \? true : undefined\}/);
  assert.match(table, /else applyDefaultView\(\)/);
  assert.match(table, /setHydratedStorageKey\(effectiveStorageKey\)/);
  assert.match(table, /return \[\.\.\.new Set\(next\)\]\.slice\(0, 3\)/);
  assert.match(table, /onMove=\{canDragColumn\(header\.column\.id\)/);
  assert.match(table, /flexRender\(header\.column\.columnDef\.header, header\.getContext\(\)\)/);
  assert.match(header, /children\?: React\.ReactNode/);
  assert.match(header, /groupingEnabled/);
  assert.match(header, /groupingLimitReached/);
  assert.match(header, /!column\.getIsGrouped\(\) && groupingLimitReached/);
  ...
  assert.match(header, /draggable/);
  assert.match(header, /aria-hidden="true"/);
  assert.match(header, /pointer:coarse/);
  assert.match(header, /GripVertical/);
  assert.match(header, /dropPosition/);
  assert.match(header, /toggleGrouping/);
