...
export const STATE_MARKER = '.create-iai-state.json';
...
export async function promptProject(flags = {}) {
  ...
  if (existedBefore) {
    ...
    if (entries.length > 0) {
      ...
      } else if (existsSync(resolve(targetDir, STATE_MARKER))) {
...
export async function fetchTemplate(ctx) {
  ...
  const tmpRoot = mkdtempSync(join(tmpdir(), 'create-iai-'));
...
export async function installTemplate(ctx) {
  ...
  if (!existsSync(envLocal)) {
    ...
    else await writeFile(envLocal, '# Generated by create-iai\n', 'utf8');
  }
  ...
  await run('git', ['commit', '-q', '-m', 'chore: initial template (create-iai)'], { cwd: dir });
